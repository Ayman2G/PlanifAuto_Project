...
class AStar:
    def __init__(self, domain, problem) -> None:
        ...
        self.landmarks = None  # Store computed landmarks here

    def calculate_landmarks_heuristic(self, state, goal):
        # Compute landmarks if not already computed
        if self.landmarks is None:
            self.landmarks = self.compute_landmarks()

        # Calculate landmarks heuristic based on computed landmarks
        landmarks_count = 0
        for landmark in self.landmarks:
            if landmark not in state.predicates:
                landmarks_count += 1
    ...
    def compute_landmarks(self):
        landmarks = set()
        all_goals = self.problem.goals

        # Consider each goal predicate as a potential landmark
        for goal in all_goals:
            # Try to achieve this goal using a relaxed plan search
            relaxed_plan = self.a_star_search_to_achieve_goal(goal)
            if relaxed_plan is not None:
                # Add achieved predicates in the relaxed plan as landmarks
                for pred in relaxed_plan:
                    landmarks.add(pred)
    ...
    def distance(self, state, goal, heuristic_name):
        ...
        if heuristic_name == "fast_forward":
            # Calculate the number of unsatisfied goals using relaxed plan length (FastForward heuristic)
            state_predicates = state.predicates
            goal_predicates = goal.predicates
            remaining_goals = [g for g in goal_predicates if g not in state_predicates]
            relaxed_plan_length = len(remaining_goals)  # Length of relaxed plan
        ...
        if heuristic_name == 'landmarks heuristic':
            distance = self.calculate_landmarks_heuristic(state, goal)
        ...
        else : ValueError(f"Unknown heuristic: {heuristic_name}")
        return distance
