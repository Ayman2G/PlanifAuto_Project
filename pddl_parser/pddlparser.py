...
from parser_files import problem
...
def t_error(t):
    print("Error: illegal character '{}' in line '{}'".format(
        t.value[0], t.lexer.lineno-1))
    t.lexer.skip(1)
...
def p_domain(p):
    ...
    p[0] = domain.Domain(name, requirements, types,
                         constants, predicates, actions)
...
def p_action_def(p):
    ...
    p[0] = domain.Action(p[3], parameters, precondition,
                         effect, probabilistic, oneof)
...
def p_precond_def(p):
    ...
    p[0] = ['PRECONDITION_KEY', domain.Precondition(
        tuple(literals), tuple(universal), tuple(existential))]
    ...
    for effect in effects:
        if 'FORALL_KEY' in effect:
            ...
            for eff in effect[1]:
                ...
                for e in eff[1]:
                    if 'FORALL_KEY' in e:
                        for_lst.append(tuple(e[1:]))
                    elif 'WHEN_KEY' in e:
                        whn_lst.append(tuple(e[1:]))
                    else:
                        lit_lst.append(e)
                prob_lst.append((eff[0], domain.Effect(
                    tuple(lit_lst), tuple(for_lst), tuple(whn_lst))))
            ...
            for eff in effect[1]:
                ...
                for e in eff:
                    if 'FORALL_KEY' in e:
                        ...
                        lit_lst.append(e)
                nd_lst.append(domain.Effect(tuple(lit_lst),
                              tuple(for_lst), tuple(whn_lst)))
            oneof.append(tuple(nd_lst))
        else:
            literals.append(effect)

    p[0] = ['EFFECT_KEY', (domain.Effect(tuple(literals), tuple(
        forall), tuple(when)), tuple(probabilistic), tuple(oneof))]
