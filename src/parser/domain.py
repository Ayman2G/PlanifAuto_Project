                ...
                if ex_action[0] in nd_actions:
                    for i in range(nd_actions[ex_action[0]]):
                        action.effects.literals = \
                            tuple(set(action.effects.literals + tuple([(-1, (('disallowed_{}'.format(
                                '%s_%s' % (map_actions[ex_action[0]], i)),) + ex_action[1:6]))])))
                else:
                    action.effects.literals = \
                        tuple(set(action.effects.literals + tuple(
                            [(-1, (('disallowed_{}'.format(ex_action[0]),) + ex_action[1:6]))])))
...
class GroundedPrecondition(object):
    ...
    def __str__(self):
        ...
        if self.pos_preconditions:
            precond_str += '\n       -- pos_preconditions: {}'.format(
                self.pos_preconditions)
        if self.neg_preconditions:
            precond_str += '\n       -- neg_preconditions: {}'.format(
                self.neg_preconditions)
...
class Action(object):
    ...
    def __str__(self, body=True):
        arglist = ', '.join(
            ['%s - %s' % pair for pair in zip(self.arg_names, self.types)])
        operator_str = '\n--------------------------------------------------'
...
class _GroundedAction(object):
    ...
    def __init__(self, action, *args, static_predicates=[]):
        ...
        def _precondition_grounder(precondition):
            ...
            if action.preconditions:
                ...
                for pre in action.preconditions.literals:
                    if pre[0] == -1:
                        if pre[1][0] in static_predicates:
                ...
                for precondition in action.preconditions.universal:
                    ...
                    universal_preconditions.append(
                        (precondition[0], tuple(pos_pre_lst), tuple(neg_pre_lst)))
                ...
                for precondition in action.preconditions.existential:
                    ...
                    for pre in precondition[1]:
                        if pre[0] == -1:
                            ...
                            pos_pre_lst.append(ground(pre))
                    existential_preconditions.append(
                        (precondition[0], tuple(pos_pre_lst), tuple(neg_pre_lst)))
            ...
            if effects:
                ...
                for effect in effects.forall:
                    ...
                    if len(effect) == 2:
                        ...
                        forall_effects.append(
                            (effect[0], tuple(pos_eff_lst), tuple(neg_eff_lst)))
                    # (forall (var_lst) (when (cnd) (effects)))
                    elif len(effect) == 3:
                        (pos_cnd_lst, neg_cnd_lst, pos_eff_lst,
                         neg_eff_lst) = ([], [], [], [])
                        for eff in effect[1]:
                            if eff[0] == -1:
                        ...
                        for eff in effect[2]:
                            if eff[0] == -1:
                                ...
                                pos_eff_lst.append(ground(eff))
                        forall_effects.append((effect[0], tuple(pos_cnd_lst), tuple(
                            neg_cnd_lst), tuple(pos_eff_lst), tuple(neg_eff_lst)))
                ...
                for effect in effects.when:
                    ...
                    for eff in effect[1]:
                        if eff[0] == -1:
                            ...
                            pos_eff_lst.append(ground(eff))
                    when_effects.append((tuple(pos_cnd_lst), tuple(
                        neg_cnd_lst), tuple(pos_eff_lst), tuple(neg_eff_lst)))
        ...
        for prob_effects in action.probabilistic:
            self.prob_effects.append(tuple([(prob_effect[0], _effect_grounder(
                prob_effect[1])) for prob_effect in prob_effects]))

        # Ground Oneof Effects
        self.oneof_effects = []
        for oneof_effects in action.oneof:
            self.oneof_effects.append(
                tuple([_effect_grounder(oneof_effect) for oneof_effect in oneof_effects]))
    ...
    def __str__(self, body=False):
        ...
        if body:
            if self.preconditions.pos_preconditions:
                operator_str += '\n-- precond+: {0}'.format(
                    ', '.join(map(str, self.preconditions.pos_preconditions)))
            if self.preconditions.neg_preconditions:
                operator_str += '\n-- precond-: {0}'.format(
                    ', '.join(map(str, self.preconditions.neg_preconditions)))
            if self.effects.add_effects:
                operator_str += '\n-- effects+: {0}'.format(
                    ', '.join(map(str, self.effects.add_effects)))
            if self.effects.del_effects:
                operator_str += '\n-- effects-: {0}'.format(
                    ', '.join(map(str, self.effects.del_effects)))
            if self.effects.when_effects:
                operator_str += '\n-- when-: {0}'.format(
                    ', '.join(map(str, self.effects.when_effects)))
