...
import src.parser.problem as problem
...
def t_error(t):
    print(
        "Error: illegal character '{}' in line '{}'".format(
...
def p_precond_def(p):
    ...
    p[0] = [
        "PRECONDITION_KEY",
        domain.Precondition(tuple(literals), tuple(universal), tuple(existential)),
    ]
...
def p_effect_def(p):
        ...
        if "FORALL_KEY" in effect:
            ...
            for eff in effect[1]:
                ...
                prob_lst.append(
                    (
                        eff[0],
                        domain.Effect(tuple(lit_lst), tuple(for_lst), tuple(whn_lst)),
                    )
                )
    ...
    p[0] = [
        "EFFECT_KEY",
        (
            domain.Effect(tuple(literals), tuple(forall), tuple(when)),
            tuple(probabilistic),
            tuple(oneof),
        ),
    ]
